...
export default async function AdminPage() {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-2xl font-bold text-foreground">Admin — Module Review</h1>
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-2">
          {recentlyReviewed.map((module) => (
            <div
              key={module.id}
              className="card-bg px-4 py-3 flex items-center justify-between"
            >
              <span className="text-sm font-medium text-foreground">{module.name}</span>
              <span
                className={module.status === "APPROVED" ? "badge-success text-xs" : "bg-red-100 text-red-700 inline-flex items-center px-3 py-1 rounded-full text-xs font-medium"}
              >
                {module.status}
              </span>
            </div>
